...
```json
{
  ...
  "to_agent": null,
  "type": "HEARTBEAT",
  "task_id": "heartbeat-aris-20260525T1000",
  "content": "gateway_ok | process_ok | tasks:TD-003(70%) | last_signal:47min | mem:65%",
  "metadata": {
    "checks": {
      "gateway": "ok",
      "process": "ok",
      "task_progress": {"TD-003": 70},
      "last_signal_minutes": 47,
      "memory_pct": 65
    }
  }
}
...
触发时机：每小时整点，agent自检cron自动触发
响应期待：无（广播性质），苏哥整点+5分钟检查完整性
通道：Nous API `/api/agents/{id}/heartbeat` + 黑板广播
...
| 信号类型 | 频率上限 | 触发方式 |
|----------|----------|----------|
| HEARTBEAT | 每小时1次（每agent） | agent自检cron自动触发 |
...
Agent的健康首先是**自检**——每个agent主动检查自身状态，报给团队。这是主动健康维护，不是被动等苏哥巡检发现问题。

Heartbeat是Nous团队的公共任务，每个agent每小时执行一次自检，结果写入Nous API和黑板。
...
| 检查项 | 方法 | 正常标准 |
|--------|------|----------|
| gateway探活 | curl自己的 `/health` endpoint | 返回200，status=ok |
| 进程存活 | 检查gateway PID | PID存在，进程运行中 |
| 当前任务进度 | 查询自己持有的kanban in_progress任务 | 有任务则报告进度%，无任务则报告idle |
| 最近信号时间 | 查Signal Log最近一条from=self的信号 | 间隔<2小时为正常 |
| 负载指标 | 进程内存/CPU占用（可选） | 内存<80%，CPU<50% |
...
```json
{
  ...
  "type": "SYNC",
  "task_id": "heartbeat-aris-20260525T1000",
  "content": "heartbeat: gateway_ok | process_ok | tasks:TD-003(70%) | last_signal:47min | mem:65%",
  "metadata": {
    "heartbeat": true,
    "checks": {
      "gateway": "ok",
...
心跳同时写入两个位置：
1. Nous API `/api/agents/{id}/heartbeat` — 更新agent_registry的last_heartbeat字段
2. 黑板 `wiki/nous/signals.md` — 追加时间线条目（全员可见）

### 自检异常处理

当自检发现问题时，heartbeat信号携带异常标记：
...
```json
{
  ...
  "type": "BLOCKED",
  "task_id": "heartbeat-aris-20260525T1000",
  "content": "heartbeat异常: gateway_timeout | last_signal:3h | 任务可能卡住",
  "metadata": {
    "heartbeat": true,
    "checks": {
      "gateway": "timeout",
      "process": "ok",
      "last_signal_minutes": 180,
      "memory_pct": 92
    },
...
**关键规则：自检异常时，agent先尝试自救，自救失败才发BLOCKED给苏哥。**

自救行为（agent本地执行，不依赖外部）：
- gateway超时：尝试重启自己的gateway进程
- 内存过高：清理缓存、释放非必要资源
- 任务卡住：检查依赖是否满足，不满足则发BLOCKED说明卡在哪

### Heartbeat时间窗口

| 时间 | 事件 | 触发方 |
|------|------|--------|
| 每小时整点 | agent自检，报heartbeat | agent自身（cron job） |
| 整点+5分钟 | 苏哥检查：哪些agent没报heartbeat | 苏哥（cron检查Nous API） |
| 整点+15分钟 | 苏哥对缺失heartbeat的agent进入管理修复流程 | 苏哥（主动介入） |
